 ...
 * @LastEditTime: 2025-02-28 12:24:35
      ...
      <div class="p-4 form-bg">
        ...
        <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="auto">
          ...
          <el-form-item size="large">
            <el-button :class="`w-100 ${!clickable?'submitBtn':''}`" :disabled="clickable" type="primary" @click="submitForm(ruleFormRef)"
              @keyup.enter.native="submitForm(ruleFormRef)">
              <text :style="`
    letter-spacing: ${locale == 'zh' ? '4px' : ''};`">{{ $t("messages.userText.formButton") }} {{ !clickable ? '' : `
                ${secondes} s` }}</text>
            </el-button>
            <el-button class="w-100 submitBtn" :disabled="clickable" @click="submitForm(ruleFormRef)"
              @keyup.enter.native="submitForm(ruleFormRef)">
              <text :style="`
