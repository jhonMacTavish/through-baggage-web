 ...
 * @LastEditTime: 2025-02-25 16:14:58
      ...
      <div class="p-4 form-bg">
        ...
        <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="auto">
          ...
          <el-form-item class="item-marginBt" size="large" :label="$t('messages.userText.formInfo.baggageNo')"
            prop="bagNo">
            <el-input v-model="ruleForm.bagNo" maxlength="9" :formatter="(value) => value.toUpperCase()">
              <template #prefix>
                <i-ep-Suitcase />
              </template>
              <template #suffix>
          ...
          <el-form-item size="large">
            <el-button class="w-100 submitBtn" type="primary" @click="submitForm(ruleFormRef)">
              <text :style="`
    letter-spacing: ${locale == 'zh' ? '4px' : ''};`">{{ $t("messages.userText.formButton") }}</text>
            </el-button>
          </el-form-item>
        </el-form>
      </div>
  ...
  <section class="p-4 flex1">
    <div class="container">
      ...
      <div class="info-content p-4 d-flex justify-content-center align-items-center">
        <p class="text-center">
          {{ $t("messages.baggageText.stateInfo.state0.str1") }}<br /><span class="attention">{{
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (valid) {
      ...
      await axios.get("http://10.86.255.9:3000/api").then((res) => {
        console.log(res);
      });
    }
  } catch (error) {
