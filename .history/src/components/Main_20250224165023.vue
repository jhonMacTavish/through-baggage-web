 ...
 * @LastEditTime: 2025-02-24 16:48:51
...
const changeLang = (type: string) => {
  ...
  rules.pptNo[0].validator = createValidator(/^[G|E|P|S|D|C]\d{8}$/, t('messages.userText.validateText.passportNo'));
  rules.fltNo[0].validator = createValidator(/^[A-Z]{2}\d{3,4}$/, t('messages.userText.validateText.flightNo'));
  rules.setNo[0].validator = createValidator(/^\d{1,3}[A-Z]$/, t('messages.userText.validateText.seatNo'));
  rules.bagNo[0].validator = createValidator(/^\d{9}$/, t('messages.userText.validateText.baggageNo'));
};
...
const rules: FormRules<typeof ruleForm> = {
  pptNo: [
    {
      validator: createValidator(/^[G|E|P|S|D|C]\d{8}$/, t('messages.userText.validateText.passportNo')),
      trigger: "blur",
    },
  ],
  fltNo: [
    {
      validator: createValidator(/^[A-Z]{2}\d{3,4}$/, t('messages.userText.validateText.flightNo')),
      trigger: "blur",
    },
  ],
  setNo: [
    {
      validator: createValidator(/^\d{1,3}[A-Z]$/, t('messages.userText.validateText.seatNo')),
      trigger: "blur",
    },
  ],
  bagNo: [
    {
      validator: createValidator(/^\d{9}$/, t('messages.userText.validateText.baggageNo')),
      trigger: "blur",
    },
  ],
};
